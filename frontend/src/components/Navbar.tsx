...
import { getOrCreateUserSession } from '../lib/api';
import { Leaf, Award, BarChart3, Calculator, MessageSquarePlus, Calendar, Sparkles, ShoppingBag, Trophy, Menu, X } from 'lucide-react';

export default function Navbar() {
  const pathname = usePathname();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [user, setUser] = useState<{ name: string; points: number; level: string } | null>(null);
  ...
  const navLinks = [
    ...
    { href: '/coach', label: 'AI Coach', icon: MessageSquarePlus },
    { href: '/challenges', label: 'Habits & Missions', icon: Calendar },
    { href: '/simulator', label: 'Tree Simulator', icon: Sparkles },
